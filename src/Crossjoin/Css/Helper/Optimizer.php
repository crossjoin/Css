...
class Optimizer
{
    public static function optimizeDeclarationValue($value)
    {
        ...
        if (strpos($value, '0')) {
            ...
            $value = preg_replace('/(?<=^|[ \r\n\t\f])(?:(?:0)?\.)?[0]+(?:in|pc|pt|cm|mm|%|em|ex|ch|rem|vw|vh|vmin|vmax|deg|grad|rad|turn|s|ms)/', '0', $value);

            // Optimize zero values: ".000 -> 0"
            $value = preg_replace('/(?<=^|[ \r\n\t\f])(?:(?:0)?\.)?[0]+(?=$|[ \r\n\t\f])/', '0', $value);

            // Optimize floats that can be written as integer: "50.0% -> 50%"
            $value = preg_replace('/\.[0]+(?=in|pc|pt|cm|mm|%|em|ex|ch|rem|vw|vh|vmin|vmax|deg|grad|rad|turn|s|ms|$|[ \r\n\t\f])/', '', $value);
        }
